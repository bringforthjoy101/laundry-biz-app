...
const getProducts = async () => {
	const response = await apiRequest({ url: '/services', method: 'GET' })
	console.log('response', response)
...
// ------------------------------------------------
mock.onGet(/\/apps\/ecommerce\/products\/\d+/).reply(async (config) => {
...
mock.onGet('/apps/ecommerce/cart').reply(async () => {
	...
	const products = data.userCart.map((cartProduct) => {
		...
		product.total = Number(product.price) * Number(cartProduct.qty)
		...
		console.log('product again', product)
...
// ------------------------------------------------
// POST: Update Item in user Cart
// ------------------------------------------------
mock.onPost('/apps/ecommerce/cart/update').reply((config) => {
	const { productId, qty } = JSON.parse(config.data)
	//find the index of object from array that you want to update
	const objIndex = data.userCart.findIndex((obj) => obj.productId === productId)
	// make new object of updated object.
	const updatedObj = { ...data.userCart[objIndex], qty }
	// make final new array of objects by combining updated object.
	const updatedCart = [...data.userCart.slice(0, objIndex), updatedObj, ...data.userCart.slice(objIndex + 1)]
	data.userCart = updatedCart
	// data.userCart.push({
	//   id: lastId + 1,
	//   productId,
	//   qty: 1
...
// ------------------------------------------------
// POST: Remove all Items in user Cart
// ------------------------------------------------
mock.onPost('/apps/ecommerce/cart/clear').reply((config) => {
	data.userCart.length = 0
	return [201]
})
